# -*- coding: utf-8 -*-
"""
Created on Fri Jan 20 23:05:30 2023
...
for i in range(10):
    ...
    #x22=dt['Health Crisis'].tolist()[:start]+dt['Health Crisis'].tolist()[end:]
    x23=dt['Depression'].tolist()[:start]+dt['Depression'].tolist()[end:]
    ...
    x22_test=dt['Health Crisis'].tolist()[start:end]'''
    x23_test=dt['Depression'].tolist()[start:end]
    ...
    coef.append(mr.coefficient(x18, mr.avg(x18), yl, ym))     x2,x5,x9,x11,x14,x23
    ...
    coef.append(mr.coefficient(x23, mr.avg(x23), yl, ym))
    
    '''calculate a value'''
    
    a=mr.calculate_A(coef,ym,mr.avg(x2),mr.avg(x5),mr.avg(x9),mr.avg(x11),mr.avg(x14),mr.avg(x23))
    
    '''linear to logistic convertion'''
    y_logistic=mr.linear_to_logistic(a,coef,x2_test,x5_test,x9_test,x11_test,x14_test,x23_test)
    '''calculate accuracy'''
    accuracy=mr.calaulate_accuracy(y_logistic, yl_test)
    
    sensitivity,specificity,accuracy1,kappa=mr.TP_TN_FP_FN_SENSITIVITY_SPECIFICITY_and_Accuracy_CALCULATE(y_logistic,yl_test)
    
    print(i+1," fold result---->")
    print("accuracy=", accuracy,"%")
    print("sensitivity=",sensitivity,"%   ","specificity=",specificity,"% ","accuracy by t=",accuracy1,"%  kappa=",kappa )
    print("\n\n")
